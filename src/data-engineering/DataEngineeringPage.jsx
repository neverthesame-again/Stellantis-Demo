...
import { dataEngineeringMockData } from "./dataEngineeringMockData";
...
export default function DataEngineeringPage({
  ...
  const currentRoleFromAuth =
    activeUser?.activeBusinessArea === "AI for Data Engineering"
      ? activeUser?.activeRole
      : null;

  const [role, setRole] = useState(
    initialRole || currentRoleFromAuth || "Data Architect"
  );
  ...
  const activeRoleData =
    dataEngineeringMockData[role] || dataEngineeringMockData["Data Architect"];
  ...
  const handleRoleSelect = (newRole) => {
    ...
    if (updateActiveContext) {
      updateActiveContext("AI for Data Engineering", newRole);
    }
  ...
  const handleDomainChange = (newDomain) => {
    ...
    if (onAreaChange) {
      ...
      if (newDomain === "AI for Modernization") {
        navigate("/modernization");
      } else if (newDomain === "AI for Data Engineering") {
  ...
  return (
    <main className="re-landing-page fade-in">
      ...
      >
        <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
          ...
          <span style={{ fontSize: "12px", color: "var(--text-secondary)" }}>
            ...
            <strong style={{ color: "#3b82f6" }}>AI for Data Engineering</strong>
          </span>
        </div>
            ...
            {(currentTabData.items || []).map((item, idx) => (
              ...
              >
                ...
                <div style={{ borderTop: "1px solid var(--border)", paddingTop: "10px", marginTop: "6px" }}>
                  ...
                  >
                    <Icon name="zap" size={14} /> Run Pipeline & Telemetry Diagnostics
                  </div>
                </div>
              </div>
            ))}
      ...
      <AdRoleBotWidget
        selectedRole={role}
        data={activeRoleData}
        domain="AI for Data Engineering"
      />
    </main>
  );
}
